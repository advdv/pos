package pos

import (
	"testing"

	"github.com/stretchr/testify/require"
)

func TestReadChaCha(t *testing.T) {
	params := NewParams(25, [32]byte{0x10})
	require.Equal(t, uint64(32204853), readChaCha(params, 30479984))
	require.Equal(t, uint64(9513690), readChaCha(params, 20)) // requires extra chacha round
}

func TestF1(t *testing.T) {
	params := NewParams(25, [32]byte{0x10})
	require.Equal(t, uint64(0b1111010110110100000110101111010), F1(params, 30479984))
}

func TestReferenceF1SingleChaCha(t *testing.T) {
	params := NewParams(25, [32]byte{0x01, 0x02, 0x2f, 0xb4, 0x2c, 0x08, 0xc1, 0x2d, 0xe3, 0xa6, 0xaf, 0x05, 0x38, 0x80, 0x19, 0x98, 0x06, 0x53, 0x2e, 0x79, 0x51, 0x5f, 0x94, 0xe8, 0x34, 0x61, 0x61, 0x21, 0x01, 0xf9, 0x41, 0x2f})
	proof, err := ProofFromHex("", 25)
	require.NoError(t, err)
	f1x1, f1x2 := F1(params, proof[0]), F1(params, proof[1])
	require.Equal(t, uint64(0b0001111100011010011100001001000), f1x1) //f1(x1)
	require.Equal(t, uint64(0b0001111100011010111001101010001), f1x2) //f1(x2)

	f1x3, f1x4 := F1(params, proof[2]), F1(params, proof[3])
	require.Equal(t, uint64(0b0111001100110101100111011000001), f1x3) //f1(x3)
	require.Equal(t, uint64(0b0111001100110110001001011010010), f1x4) //f1(x4)
}

func TestReferenceF1DoubleChaCha(t *testing.T) {
	params := NewParams(25, [32]byte{0x01, 0x02, 0x2f, 0xb4, 0x2c, 0x08, 0xc1, 0x2d, 0xe3, 0xa6, 0xaf, 0x05, 0x38, 0x80, 0x19, 0x98, 0x06, 0x53, 0x2e, 0x79, 0x51, 0x5f, 0x94, 0xe8, 0x34, 0x61, 0x61, 0x21, 0x01, 0xf9, 0x41, 0x2f})
	x1 := uint64(0b1101010010011110010111000)
	f1x1 := F1(params, x1)
	require.Equal(t, uint64(0b1100000110111000111010000110101), f1x1)
}
